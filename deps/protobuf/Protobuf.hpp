...
struct ErrorOr
{
	...
	ErrorOr(T const &val, int err) : value(val), error(err)
	{
	}
...
{
	uint8_t buf[10];
	uint8_t *end = proto::codec::writeVarint64(buf, num);
	stream.insert(stream.end(), buf, end);
}
...
public:
	...
	virtual bool IsByteArrayObject() const
	{
		return false;
	}

	// Gets the last child element with the specified field number.
	virtual ObjectBase *GetFieldObject(FieldNumber fieldNumber)
	{
		return nullptr;
	}
	...
	virtual ObjectBase *GetFieldObjectWithDefault(FieldNumber fieldNumber, ObjectBase *pObj)
	{
		delete pObj;
		return nullptr;
	}
	...
	virtual std::vector<ObjectBase *> *GetFieldObjects(FieldNumber fieldNumber)
	{
		return nullptr;
	}
	...
	virtual bool EraseField(FieldNumber fieldNumber)
	{
		return false;
	}
...
public:
	...
	ObjectBaseMessage(ObjectBaseMessage const &) =
		delete; // delete the copy constructor. You can't do that because I'm too lazy to make it possible
...
public:
	bool IsMessageObject() const override
	{
		return true;
	}

	bool IsByteStorageObject() const override
	{
		return true;
	}
...
public:
	bool IsEmbeddedMessageObject() const override
	{
		return true;
	}
	...
	size_t GetPayloadSize() const override
	{
		...
		return GetVarIntSize(CombineFieldNumberAndTag(GetFieldNumber(), TAG_LEN)) + GetVarIntSize(embPayloadSize) +
		       embPayloadSize;
	}
...
{
public:
	ObjectString(FieldNumber fn, std::string const &str = "") : ObjectBase(fn), m_content(str)
	{
	}

	ObjectString *SetContent(std::string const &str)
	{
		m_content = str;
		MarkDirty();
		return this;
...
public:
	size_t GetPayloadSize() const override
	{
		return GetVarIntSize(CombineFieldNumberAndTag(GetFieldNumber(), TAG_LEN)) + GetVarIntSize(m_content.size()) +
		       m_content.size();
	}
...
{
public:
	ObjectBytes(FieldNumber fn, std::vector<uint8_t> const &val = {}) : ObjectBase(fn), m_content(val)
	{
	}

	ObjectBytes(FieldNumber fn, char const *data, size_t sz) : ObjectBase(fn)
	{
		m_content.resize(sz);
		memcpy(m_content.data(), data, sz);
	}

	ObjectBytes(FieldNumber fn, uint8_t const *data, size_t sz) : ObjectBase(fn)
	{
		m_content.resize(sz);
		memcpy(m_content.data(), data, sz);
	}

	ObjectBytes *SetContent(std::vector<uint8_t> const &val)
	{
		m_content = val;
		MarkDirty();
		return this;
...
public:
	...
	bool IsByteArrayObject() const override
	{
		return true;
	}
...
// ===== DECODE =====
static ErrorOr<uint64_t> DecodeVarInt(char const *data, size_t &offset, size_t sz)
{
	if (offset >= sz)
	{
	...
	uint8_t const *ptr = reinterpret_cast<uint8_t const *>(data + offset);
	uint8_t const *end = reinterpret_cast<uint8_t const *>(data + sz);
	uint64_t value;

	if (!proto::codec::readVarint64(ptr, end, value))
	{
		return {0, ERROR_VARINTTOOBIG};
	}

	offset = ptr - reinterpret_cast<uint8_t const *>(data);
	return {value, 0};
}

static int DecodeBlock(char const *data, size_t sz, ObjectBaseMessage *block, DecodeHint *pHint = nullptr)
{
	...
	while (offset < sz)
	{
		...
		case TAG_LEN: {
			...
			char const *dataSubset = &data[offset];
			offset += length;
			bool isString = true;
...
static int DecodeBlock(uint8_t const *data, size_t sz, ObjectBaseMessage *block)
{
	return DecodeBlock((char const *)data, sz, block);
}
} // namespace Protobuf
